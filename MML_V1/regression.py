...
class LinearMSE(Regression):
    ...
    @classmethod
    def update_gradients(cls, data: np.array, weights: np.array) -> (float, np.array):
        """
        @brief      Find new gradients of the data given the weights
        @param data     Array of arrays of any length [[x1, x2, ..., y], [x1, x2, ... y], ...]
        @param weights   Array of weights (thetas)
        """
        ...
        # Insert 1 to front of each data sample for dot product
        temp_data = np.concatenate((data, np.ones((data.shape[0], 1))), axis=1)
        # Append 1 to end of weights for dot product
        temp_weights = np.append(weights, 1, 0)

        errors = np.dot(temp_data, temp_weights)  # Array of errors for each n samples

        # Insert a sample of all 1s to the front of the array and take off they ys
        temp_x = np.concatenate((data, np.ones((data.shape[1], 0))), axis=0)[:, :-1]
        gradients = 1/len_data * np.append(temp_x, errors)
...
class LogisticBinaryCrossEntropy(Regression):
    ...
    def calculate_error_and_gradients(cls, data: np.array, weights) -> (float, np.array):
        ...
        cross_entropy = 0
        len_data = len(data)
        # prevents taking log of 0 (Used ChatGPT to diagnose issue)
        epsilon = 1e-10
        ...
        for i in range(len_data):
            ...
            # Cross Entropy Error
            error = -y * math.log(y_hat + epsilon) - \
                (1 - y) * math.log(1-y_hat + epsilon)
